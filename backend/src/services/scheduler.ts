  ...
  const results = await Promise.allSettled(
    targets.map(async (target) => {
      ...
      switch (account.platform) {
        ...
        case 'youtube': {
          // YT rejects titles with < or >, those over 100 chars, or that
          // collapse to empty after trimming. IG captions sometimes carry
          // BOM / right-to-left marks / zero-width chars that look fine
          // but fail YT validation, so sanitise here.
          const rawTitle = (post.caption || '').split('\n')[0] || '';
